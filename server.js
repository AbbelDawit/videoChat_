...
io.on("connection", (socket) => {
  ...
  socket.on("nextButtonClick", (data) => {
    // Broadcast the "Next" button click event to all connected users
    io.emit("nextButtonClick", data);
  });


/*
  socket.on("reportUser", (data) => {
    console.log("User Reported Recieved: ", data);
    // Notify the reporting user that the report was successful
    io.emit("reportSuccess", { message: "User reported successfully."});
  });
*/
});
